...
<img src="docs/hero.png" alt="ThoughtDAG 地图视角：一个问题、四个分区章节，每张卡收起成一句收获，关键动作带徽章（排除、决策、转向、待解），阅读闭环章里有带封面的 PDF 和导读节点" width="100%"/>
...
<img src="docs/reader.png" alt="材料阅读器：被提问的句子在原版页面上带着高亮和气泡，批注栏展示那条对话，顶部有导读标签页，底部是带页码标签的问题 chips" width="100%"/>
...
- **材料阅读器**: 原版 PDF 渲染+可选中文字层（pdf.js）；圈选→提问=支线节点带 `(p.N)` 出处，段落在页面留下锚点（高亮洗染+重开对话的气泡）；画布节点带 p.N 芯片反向跳回阅读器；扫描件回退提取文本视图；逐页视觉**识别**为 Markdown/LaTeX（可编辑；外部 OCR 工具的输出可直接贴入）；整份材料提问常驻输入；底部对话索引给每条线程标注 p.N 或整篇；每份材料记住滚动位置
