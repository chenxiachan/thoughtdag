...
**[▶ 在线试用](https://thoughtdag.xia-chen.workers.dev)**，无需安装、无需注册。带上任何兼容 OpenAI 协议的 key（它只留在你的浏览器里），或者直接逛示例画布。
...
| 🧹 收敛 | 框选冗余节点，合并成一个结构化综合（可附加意图定向），原件归档 |
...
- **数据在你手里。** 画布存在浏览器 IndexedDB；唯一的服务端是你自己机器上的轻代理。除了你选择的 LLM API，数据不会发往任何别的地方。在线 Demo 上，模型流量从浏览器直连网关，key 和对话完全不经过 Demo 的服务器。备份是你完全拥有的 JSON 文件，自动文件夹备份让它们始终保持最新。
