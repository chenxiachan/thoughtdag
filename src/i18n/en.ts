...
export const en = {
  ...
  'tutorial.step1.desc': 'A question is a node: your question plus the AI\'s answer. Type below, double-click empty canvas, or drop in a PDF.',
  'tutorial.step2.title': '2 · Follow up',
  'tutorial.step2.desc': 'A follow-up grows a solid-wired child. What the AI sees is exactly what flows in along the arrows. The one rule: a wire IS context.',
  ...
  'tutorial.step4.desc': 'Click a wire, delete it — that history leaves the context. Drag a wire between nodes to merge threads. The will-send preview shows the exact context anytime.',
  'tutorial.step5.title': '5 · Zoom out to the map',
  'tutorial.step5.desc': 'Zoom out: every card folds to its one-line takeaway, decisive moves wear badges — ✕ ruled out · ⚖ decided · ↩ pivoted · ? open. Zoom further: just the seals. Shape from afar, full text up close.',
  'tutorial.step6.title': '6 · Material & the reader',
  'tutorial.step6.desc': 'Paste becomes a note, a URL a snapshot, a PDF a file node. Double-click to read: select any passage to ask, answers land back page-anchored. Unwired material stays out of context.',
  'tutorial.step7.title': '7 · References',
  'tutorial.step7.desc': 'Drag from a node handle: onto an existing node makes a dashed reference — just that Q&A, with a token price tag, quote ⇄ full. Onto empty canvas: ask right there.',
  'tutorial.step8.title': '8 · Batch & frames',
  'tutorial.step8.desc': 'Marquee-select nodes: merge into a summary, ask them together, duplicate the block, archive. Frame a chapter — drag the frame and its nodes come along.',
  'tutorial.step9.title': '9 · Staleness & replay',
  'tutorial.step9.desc': 'Upstream changed? Downstream wears an amber badge: this answer predates it. Click to re-answer in place (old versions kept); the toolbar replays everything, priced first.',
  'tutorial.step10.title': '10 · Share & paradigms',
  'tutorial.step10.desc': 'Share makes a read-only link: the data travels in the link itself, no server, viewers browse but cannot edit. Paradigms save a whole workflow as a template — new input, same experiment.',
  ...
  'content.noVisionModel': 'No vision model configured: images can\'t be auto-understood. Add a vision-capable key in .env (the .env.example template marks the free options) and restart npm run server.',
  ...
  'caps.webSearchOff': 'Needs ZHIPU_API_KEY in .env for server-side search. The search toggle stays hidden until then.',
  ...
  'caps.visionOff': 'Needs at least one vision model (.env.example marks the free options). Image auto-reading and Recognize stay hidden until then.',
