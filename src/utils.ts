...
export function getContextPath(
  ...
  // DAG traversal — same logic as buildContext in store.ts
  ...
  function walkUp(id: string) {
    ...
    if (!node) return;
    // Follow ALL incoming edges (blue, orange, cross-link)
...
function estimateNodeHeight(node: ThoughtNode): number {
  if (node.data.isCollapsed) return 80;
...
export function autoLayout(nodes: ThoughtNode[], edges: ThoughtEdge[]): ThoughtNode[] {
  ...
  // Get parent's position (if any)
  const parentMap = new Map<string, string>();
  for (const edge of structuralEdges) {
    // last writer wins — fine for trees; for DAG the structural parent is sufficient
  ...
  for (let pass = 0; pass < 5; pass++) {
    ...
    // Also check cross-column overlaps (nodes at similar y in adjacent columns)
    // This is lighter — just ensure branch children don't overlap vertically
    // with nodes in their column that come from other parents
    if (!moved) break;
  }
