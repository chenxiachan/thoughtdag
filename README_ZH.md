...
模型不替你划重点：高光是你亲手圈选、判断过的内容。总览按时间或按节点一处看全，每条可定位出处；勾选任意几条，串成每句可溯源的一段话。
...
<table>
<tr>
<td width="45%"><img src="docs/illus/replay-zh.svg" alt="示意图：上游节点刚被编辑，下游回答亮起琥珀色陈旧标记，重放前显示 token 报价；旁边一张已归档的淡卡"/></td>
<td width="55%">

### 🧭 每一次生成，都由你按下

上游一改，受影响的回答立刻亮起标记；重放哪些、按什么顺序、花多少 token，先报价、后执行。归档的节点退出一切未来上下文。**每条边、每次生成，都是人的决定。**
