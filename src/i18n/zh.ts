...
export const zh: Record<keyof typeof en, string> = {
  ...
  'tutorial.step2.desc': '追问长出实线相连的子节点。AI 看到的就是沿箭头流入的全部上游。唯一法则：连线即上下文。',
  ...
  'condense.copyIntro': '生成一棵「凝练副本」放在原树右侧：勾选的连续段坍缩成蒸馏节点（决策与结论清单，高光原文保真），其余节点原样复制、连线照接。原树一个字都不动，随时对比；满意后可归档原树。决策/转向徽章的节点永不坍缩。',
  ...
  'condense.copyCompareHint': '两棵树并排对比：同一个问题分别连到两边问一次，答案和 token 价签见分晓。蒸馏节点上的芯片可跳回原段。满意后归档原树（淡化显示，随时找回）。',
